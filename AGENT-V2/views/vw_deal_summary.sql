...
SELECT
    ...
    CAST(MAX(TR.LAST_SETTLEMENT) AS TIMESTAMP(6)) AS SETTLEMENT_TS,
    CAST(NULL AS VARCHAR2(4000)) AS SETTLEMENT_CURRENCY,
    CAST(MAX(TR.TRANCHE_COUNT) AS NUMBER(38,0)) AS TRANCHE_COUNT,
    NVL(MAX(TR.FIRST_PRICED), MAX(II.PRICING_DT)) AS FIRST_PRICED,
    NVL(MAX(TR.LAST_PRICED), MAX(II.PRICING_DT)) AS LAST_PRICED,
...
LEFT JOIN (
    SELECT O.DEAL_ID,
           COUNT(DISTINCT O.ORDER_ID) AS ORDER_COUNT,
           COUNT(DISTINCT O.INVESTOR_GPNUM) AS INVESTOR_COUNT,
           CAST(SUM(CASE WHEN O.IOI_UNIT IN ('SHARES', 'BOND')
