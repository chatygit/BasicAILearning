...
SELECT
    ...
    MAX(OD.TOTAL_DEMAND) AS TOTAL_DEMAND,
    MAX(OD.TOTAL_ALLOCATION) AS TOTAL_ALLOCATION,
    CASE WHEN MAX(T.DEAL_SIZE) > 0
         THEN ROUND(MAX(OD.TOTAL_DEMAND) / MAX(T.DEAL_SIZE), 2)
         END AS SUBSCRIPTION_RATIO,
    MAX(OBT.DEAL_FEE_MM) AS DEAL_FEE_MM,
    MAX(OBT.DEAL_FEE_CURRENCY) AS DEAL_FEE_CURRENCY,
    MAX(OBT.DEAL_SIZE_MM) AS DEAL_SIZE_MM,
    MAX(OBT.DEAL_SIZE_CURRENCY) AS DEAL_SIZE_CURRENCY,
    MAX(CAST(T.ANNOUNCE_TS AS TIMESTAMP(6))) AS FIRST_ANNOUNCED,
    MAX(T.BASE_PRICE) AS BASE_PRICE,
    MAX(T.REOFFER_LOW_PRICE) AS REOFFER_LOW_PRICE,
    MAX(T.REOFFER_HIGH_PRICE) AS REOFFER_HIGH_PRICE,
    MAX(T.FX_RATE) AS FX_RATE,
...
LEFT JOIN (
    SELECT O.DEAL_ID,
           ...
           SUM(COALESCE(CAST(O.DEMAND_QTY AS NUMBER),
                        CAST(OI.LIMIT_VALUE AS NUMBER), 0)) AS TOTAL_DEMAND,
...
SELECT
    ...
    OC.INVESTOR_COUNT AS INVESTOR_COUNT,
    D.TRANSACTION_ID AS TRANSACTION_ID,
    CAST(NULL AS VARCHAR2(200)) AS ISSUER_LEI,
    OC.TOTAL_DEMAND AS TOTAL_DEMAND,
    OC.TOTAL_ALLOCATION AS TOTAL_ALLOCATION,
    CASE WHEN D.DEAL_SIZE > 0
         THEN ROUND(OC.TOTAL_DEMAND / D.DEAL_SIZE, 2)
         END AS SUBSCRIPTION_RATIO,
    CAST(NULL AS NUMBER) AS DEAL_FEE_MM,
    CAST(NULL AS VARCHAR2(400)) AS DEAL_FEE_CURRENCY,
    CAST(NULL AS NUMBER) AS DEAL_SIZE_MM,
    CAST(NULL AS VARCHAR2(400)) AS DEAL_SIZE_CURRENCY,
    CAST(D.FIRST_ANNOUNCED AS TIMESTAMP(6)) AS FIRST_ANNOUNCED,
    CAST(NULL AS NUMBER) AS BASE_PRICE,
    CAST(NULL AS NUMBER) AS REOFFER_LOW_PRICE,
    CAST(NULL AS NUMBER) AS REOFFER_HIGH_PRICE,
    CAST(NULL AS NUMBER) AS FX_RATE,
