...
SELECT
    ...
    CAST(NVL(NVL(RT.TRN_UW_SIZE_QTY, RT.ACTIVE_TRANCHE_SIZE_QTY), 0) AS NUMBER(38,4)) AS TRANCHE_SIZE,
    CAST(NVL(NVL(NVL(TT.PRICING_TS, II.PRICING_DT), II.OFFER_DT), TT.TRADE_DATE) AS TIMESTAMP(3)) AS PRICING_TS,
    ...
    NVL(DST.DEAL_SHARING_TYPE, 'SHARED') AS DEAL_SHARING_TYPE,
    T.PRODUCT_EQUITY_TYPE_VALUE AS EQUITY_TYPE,
    CAST(NVL(TT.SETTLEMENT_TS, II.SETTLEMENT_DT) AS TIMESTAMP(6)) AS SETTLEMENT_TS,
    T.DEAL_TRANSACTION_ID AS TRANSACTION_ID,
    CAST(NULL AS VARCHAR2(200)) AS ISSUER_LEI,
    CAST(NULL AS VARCHAR2(80)) AS COUPON,
    CAST(NULL AS VARCHAR2(400)) AS YIELD,
    CAST(NVL(TPD.FINAL_PRICE, PP.OFFER_PX) AS NUMBER(38,6)) AS PRICE,
    CAST(NULL AS VARCHAR2(4000)) AS PRICE_GUIDANCE,
    CAST(NULL AS VARCHAR2(400)) AS ORDER_BOOK_SIZE_USD,
    CAST(ROUND(NVL(PF.GROSS_SPREAD_AMT, PF.U_W_FEE_AMT + PF.MGMT_FEE_AMT + PF.SELLING_CONC_FEE_AMT), 6) AS NUMBER(38,6)) AS TOTAL_FEE,
    CAST(ROUND(PF.U_W_FEE_AMT, 6) AS NUMBER(38,6)) AS UNDERWRITING_FEE,
    CAST(ROUND(PF.MGMT_FEE_AMT, 6) AS NUMBER(38,6)) AS MANAGEMENT_FEES,
    CAST(ROUND(NVL(TT.SELLING_CONCESSION_FEE, PF.SELLING_CONC_FEE_AMT), 6) AS NUMBER(38,6)) AS SELLING_CONCESSION_FEE,
...
LEFT JOIN (
    SELECT
        S.ECM_TRANSACTION_ID,
        S.ECM_TRANSACTION_TRANCHE_ID,
        LISTAGG(CASE WHEN REGEXP_LIKE(S.SYNDICATE_MEMBER_NAME, '^CITI(BANK|GROUP|USA|US[0-9]|UKE|BRAS|CAN|ASIA|AUS|SEC|INVS|G[A-Z]*|[0-9]+)?$', 'i') THEN 'Citigroup (' || S.SYNDICATE_MEMBER_NAME || ')' ELSE S.SYNDICATE_MEMBER_NAME END, ' | ' ON OVERFLOW TRUNCATE '...' WITH COUNT) WITHIN GROUP (ORDER BY S.SYNDICATE_MEMBER_NAME, S.SYNDICATE_ROLE) AS SYNDICATE_MEMBER_NAME,
...
LEFT JOIN (
    SELECT
        ...
        CASE
            WHEN COUNT(S.SYNDICATE_MEMBER_NAME) > 0
                AND SUM(CASE WHEN REGEXP_LIKE(S.SYNDICATE_MEMBER_NAME, '^CITI(BANK|GROUP|USA|US[0-9]|UKE|BRAS|CAN|ASIA|AUS|SEC|INVS|G[A-Z]*|[0-9]+)?$', 'i')
                             THEN 0 ELSE 1 END) = 0
            THEN 'SOLO'
            ELSE 'SHARED'
        END AS DEAL_SHARING_TYPE
...
LEFT JOIN (
    SELECT V.*
    FROM (
        SELECT R.TRN_ID, R.TRN_NM, R.ACTIVE_TRANCHE_SIZE_QTY, R.TRN_UW_SIZE_QTY, R.OVERALLOTMENT_QTY, R.DEFAULT_PRD_ID,
...
LEFT JOIN (
    SELECT PRD_ID, MAX(GROSS_SPREAD_AMT) AS GROSS_SPREAD_AMT, MAX(U_W_FEE_AMT) AS U_W_FEE_AMT,
           MAX(MGMT_FEE_AMT) AS MGMT_FEE_AMT, MAX(SELLING_CONC_FEE_AMT) AS SELLING_CONC_FEE_AMT
    FROM DGSTREAM.IPREO_PRODUCTFEE
    WHERE PRD_ID IS NOT NULL
    GROUP BY PRD_ID
) PF
    ON PF.PRD_ID = RT.DEFAULT_PRD_ID
LEFT JOIN (
    SELECT PRD_ID, MAX(OFFER_PX) AS OFFER_PX
    FROM DGSTREAM.IPREO_PRODUCT
    WHERE PRD_ID IS NOT NULL
    GROUP BY PRD_ID
) PP
    ON PP.PRD_ID = RT.DEFAULT_PRD_ID
LEFT JOIN (
