...
BAD_SQL = [
    ...
    ("trace-0804-grouped-without-deal-id", "SELECT DEAL_NAME, OFFERING_TYPE, PRICING_TS, COUNT(DISTINCT GPNUM) AS N FROM DGSTREAM.VW_DEAL_ORDER_SUMMARY WHERE DEAL_ID = '25240222' AND PRODUCT = 'ECM' GROUP BY DEAL_NAME, OFFERING_TYPE, PRICING_TS", "duplicate-prone"),
]
...
GOOD_SQL = [
    ...
    ("trace-0804-pure-aggregate-one-row", "SELECT MAX(DEAL_NAME) AS DEAL_NAME, MAX(OFFERING_TYPE) AS OFFERING_TYPE, MIN(PRICING_TS) AS PRICING_TS, COUNT(DISTINCT GPNUM) AS N FROM DGSTREAM.VW_DEAL_ORDER_SUMMARY WHERE DEAL_ID = '25240222' AND PRODUCT = 'ECM'"),
    ("trace-0804-grouped-by-deal-id", "SELECT DEAL_NAME, MAX(OFFERING_TYPE) AS O, MIN(PRICING_TS) AS P, COUNT(DISTINCT GPNUM) AS N FROM DGSTREAM.VW_DEAL_ORDER_SUMMARY WHERE DEAL_ID = '25240222' AND PRODUCT = 'ECM' GROUP BY DEAL_ID, DEAL_NAME"),
]
...
INVARIANTS = [
    ...
    (SKILL, "AN ENTITY REFERENCE IS ONE STRING", "never decompose a name into id+name (wasted hop)"),
