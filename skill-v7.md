...
7. **Pipe-list cells are ATOMIC** — never split a pipe-delimited value across table columns (it shifts every column after it). Aligned TYPE|VALUE pairs (identifiers, brokers+roles) get ZIPPED by position into one cell: "CUSIP 123456789 · FIGI 12345X". NULL cells → "—".
