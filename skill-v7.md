...
- **Umbrella names** (fidelity, blackrock, vanguard, state street, jpmorgan…) usually mean several legal entities: if search returns multiple GPNUMs → numbered options **shown most-active first with the activity hint (deal_count, last_active from the results)** — dormant/junk variants sink; if the user's pick then returns ZERO rows for their window, offer the most-active sibling from the same list ("BLACKROCK had no 2025 activity — BLACKROCK INC shows 34 deals; try that?") (`1) BLACKROCK FUND ADVISORS [GPNUM: 12345]` — "reply with number or id"), wait. **Exception: user typed a full legal-entity name and search returns ONE exact match → proceed, don't ask.** Umbrella caution applies to investor/issuer intent only — in syndicate/bookrunner/billed context those names are brokers (§7), zero resolution.
...
- **Billed by ANY bank — canonical recipe (both branches by default)**:
  `( PRODUCT='ECM' AND REGEXP_LIKE(SYNDICATE_MEMBER_NAME, '<bank>[^|]*\(true\)', 'i') ) OR ( PRODUCT='DCM' AND UPPER(SYNDICATE_MEMBER_NAME) LIKE '%<BANK>%' )` — Citi stem: CITIGROUP GLOBAL MARKETS. The `(true)` flag lives INSIDE the same member token (alignment-free attribution). ⚠ ANTI-PATTERN: `LIKE '%<bank>% (TRUE)'` — the % between bank and flag matches whichever bank is LAST in the list → wrong/zero rows. ⚠ 'Bill and Deliver' also exists as a SYNDICATE_ROLE value, but roles are POSITION-ALIGNED with members — you cannot attribute WHICH bank via role LIKE + member LIKE; display only, never a "X was B&D" filter.
